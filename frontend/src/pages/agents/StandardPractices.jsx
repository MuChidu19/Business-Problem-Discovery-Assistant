...
export default function StandardPractices() {
  ...
  const onAnalyze = async () => {
    ...
    try {
      ...
      const res = await analyze('standard-practices', payload)
  ...
  return (
    <>
      <SharedHeader title="Standard Practices Agent" subtitle="Explore industry standards and company practices" />
      <div className="container">
        ...
        <div style={{marginTop: 12}}>
          <button className="button primary" disabled={!canAnalyze || loading} onClick={onAnalyze}>
            {loading ? 'Researching…' : 'Analyze Standard Practices'}
          </button>
        </div>
        {html && <SectionTitle title="Standard Practices" />}
        {html && (
          <div style={{margin:'8px 0 0', textAlign:'center', opacity:0.8}}>
            Industry and company practices for <strong>{form.account || 'Unknown Company'}</strong> in <strong>{form.industry || 'Unknown Industry'}</strong>.
          </div>
        )}
        <div style={{marginTop: 16}} className="card fade-up">
          <div dangerouslySetInnerHTML={{ __html: html }} />
        </div>
        <ReportActions agentDisplayName="Standard Practice Agent" account={form.account} htmlContent={html} />
