...
export default function QuestionDiscovery() {
  ...
  const onAnalyze = async () => {
    ...
    try {
      ...
      const res = await analyze('question-discovery', payload)
  ...
  return (
    <>
      <SharedHeader title="Question Discovery" subtitle="Generate structured, insightful discovery questions" />
      <div className="container">
        ...
        <div style={{marginTop: 12}}>
          <button className="button primary" disabled={!canAnalyze || loading} onClick={onAnalyze}>
            {loading ? 'Analyzing…' : 'Discover Questions'}
          </button>
        </div>
        {html && <SectionTitle title="Question Discovery" />}
        {html && (
          <div style={{margin:'8px 0 0', textAlign:'center', opacity:0.8}}>
            Discovery questions for <strong>{form.account || 'Unknown Company'}</strong>.
          </div>
        )}
        <div style={{marginTop: 16}} className="card fade-up">
          <div dangerouslySetInnerHTML={{ __html: html }} />
        </div>
        <ReportActions agentDisplayName="Question Discovery Agent" account={form.account} htmlContent={html} />
