...
session_defaults = {
    ...
    'uncertainty_feedback_submitted': False,
...
API_CONFIGS = [
    {
        ...
        "prompt": lambda problem, outputs: (
            ...
            f"Score 0â€“5. Provide justification."
        )
    },
    {
        "name": "Q11",
...
def submit_feedback(feedback_type, employee_id="", off_definitions="", suggestions="", additional_feedback=""):
    ...
    save_feedback_to_admin_session(admin_data, "Uncertainty Agent")
...
if st.session_state.get("show_uncertainty") and st.session_state.get("uncertainty_outputs"):
        ...
        <div style="margin:20px 0;">
            <div class="section-title-box" style="padding:1rem 1.5rem; background:#8b1e1e; border-radius:12px;">
                <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; color:white;">
                    <h3 style="margin:0; font-weight:800; font-size:1.4rem;">Uncertainty Analysis</h3>
    ...
    if not st.session_state.get('uncertainty_feedback_submitted', False):
        fb_choice = st.radio(
            ...
            index=None,
            key="uncertainty_feedback_radio",
        )
        ...
        if fb_choice == "I have read it, found it useful, thanks.":
            ...
            with st.form("uncertainty_feedback_form_analyses", clear_on_submit=True):
                ...
                for api_name in st.session_state.uncertainty_outputs.keys():
                    api_desc = next((cfg["description"] for cfg in API_CONFIGS if cfg["name"] == api_name), api_name)
                    selected = st.checkbox(f"**{api_name}** - {api_desc}", key=f"uncertainty_issue_{api_name}")
                    if selected:
                        selected_issues[api_name] = True
                additional_feedback = st.text_input("Additional comments:", key="uncertainty_analyses_additional")
            ...
            with st.form("uncertainty_feedback_form_suggestions", clear_on_submit=True):
                ...
                st.markdown(f'**Employee ID:** {employee_id}')
                suggestions = st.text_input("Your suggestions:", key="uncertainty_suggestions_text")
                if st.form_submit_button("Submit Feedback"):
        ...
        if st.button("Submit Additional Feedback", key="uncertainty_reopen_feedback_btn", use_container_width=True):
    ...
    # DOWNLOAD SECTION
    # =========================================
    if st.session_state.get('uncertainty_feedback_submitted', False):
        st.markdown("---")
        st.markdown(
            """
            <div style="margin: 10px 0;">
                <div class="section-title-box" style="padding: 0.5rem 1rem;">
                    <div style="display:flex; flex-direction:column; align-items:center; justify-content:center;">
                        <h3 style="margin:0; color:white; font-weight:700; font-size:1.2rem;">Download Uncertainty Analysis</h3>
                    </div>
                </div>
            </div>
            """,
...
"""
            st.download_button(
                "Download Uncertainty Analysis as Text File",
