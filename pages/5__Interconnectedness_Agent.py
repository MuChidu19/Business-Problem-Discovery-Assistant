...
# =========================================
session_defaults = {
    'interconnectedness_outputs': {},
    'show_interconnectedness': False,
    'interconnectedness_feedback_submitted': False,
...
API_CONFIGS = [
    ...
    },
    {
        ...
        "prompt": lambda problem, outputs: (
            ...
            f"Context from vocabulary:\n{outputs.get('vocabulary', '')}\n\n"
            f"Context from current system:\n{outputs.get('current_system', '')}\n\n"
            "How complex is the problem in terms of stakeholders, processes, or technology involved?\n\n"
            "Score 0–5. Provide justification."
        )
    },
    {
        ...
        "prompt": lambda problem, outputs: (
            ...
            f"Context from vocabulary:\n{outputs.get('vocabulary', '')}\n\n"
            f"Context from current system:\n{outputs.get('current_system', '')}\n\n"
            "How dependent is the problem on external factors or third parties?\n\n"
            "Score 0–5. Provide justification."
        )
    }
]
...
def submit_feedback(feedback_type, employee_id="", off_definitions="", suggestions="", additional_feedback=""):
    ...
    save_feedback_to_admin_session(admin_data, "Interconnectedness Agent")
    ...
    st.session_state.interconnectedness_feedback_submitted = True
    return True
...
if analyze_btn:
    ...
    with st.spinner("Analyzing system dependencies and relationships..."):
        ...
        for i, api_cfg in enumerate(API_CONFIGS):
            ...
            st.session_state.interconnectedness_outputs[api_cfg["name"]] = result if result else "No data available"
...
if st.session_state.get("show_interconnectedness") and st.session_state.get("interconnectedness_outputs"):
        ...
        <div style="margin:20px 0;">
            <div class="section-title-box" style="padding:1rem 1.5rem; background:#8b1e1e; border-radius:12px;">
                <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; color:white;">
                    <h3 style="margin:0; font-weight:800; font-size:1.4rem;">Interconnectedness Analysis</h3>
    ...
    for api_name, api_output in st.session_state.interconnectedness_outputs.items():
        if api_output and api_output != "No data available":
            ...
            formatted_html = format_interconnectedness_with_bold(api_output)
    ...
    if not st.session_state.get('interconnectedness_feedback_submitted', False):
        fb_choice = st.radio(
            ...
            index=None,
            key="interconnectedness_feedback_radio",
        )

        if fb_choice == "I have read it, found it useful, thanks.":
            with st.form("interconnectedness_feedback_form_positive", clear_on_submit=True):
            ...
            with st.form("interconnectedness_feedback_form_analyses", clear_on_submit=True):
                st.markdown("**Please select which analyses seem off:**")
                st.markdown(f'**Employee ID:** {employee_id}')
                selected_issues = {}
                for api_name in st.session_state.interconnectedness_outputs.keys():
                    api_desc = next((cfg["description"] for cfg in API_CONFIGS if cfg["name"] == api_name), api_name)
                    selected = st.checkbox(f"**{api_name}** - {api_desc}", key=f"interconnectedness_issue_{api_name}")
                    if selected:
                        selected_issues[api_name] = True
                additional_feedback = st.text_input("Additional comments:", key="interconnectedness_analyses_additional")
            ...
            with st.form("interconnectedness_feedback_form_suggestions", clear_on_submit=True):
                st.markdown("**Please share your suggestions:**")
                st.markdown(f'**Employee ID:** {employee_id}')
                suggestions = st.text_input("Your suggestions:", key="interconnectedness_suggestions_text")
                if st.form_submit_button("Submit Feedback"):
        ...
        if st.button("Submit Additional Feedback", key="interconnectedness_reopen_feedback_btn", use_container_width=True):
            st.session_state.interconnectedness_feedback_submitted = False
            st.rerun()

    # =========================================
    # DOWNLOAD SECTION
    # =========================================
    if st.session_state.get('interconnectedness_feedback_submitted', False):
        st.markdown("---")
        st.markdown(
            """
            <div style="margin: 10px 0;">
                <div class="section-title-box" style="padding: 0.5rem 1rem;">
                    <div style="display:flex; flex-direction:column; align-items:center; justify-content:center;">
                        <h3 style="margin:0; color:white; font-weight:700; font-size:1.2rem;">Download Interconnectedness Analysis</h3>
                    </div>
                </div>
            </div>
            """,
...
"""
            st.download_button(
                "Download Interconnectedness Analysis as Text File",
